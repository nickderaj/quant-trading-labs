...
| [025](src/research/025_pricing_bespoke_commodity_derivatives.ipynb) | [Pricing Bespoke Commodity Derivatives](src/results/025_pricing_bespoke_commodity_derivatives.md) | A teaching notebook, not a forecasting contest: eight pricing methods cross-validated against each other (17/17 pass, including two 1e-8/1e-10-tolerance identities) with no option prices anywhere in this repo, then applied to seven real hedging problems. Black-76 assigns the actual 2020-04-20 negative-WTI settlement zero probability (log score −∞); the farmer's cheapest knock-out floor would have fired on 28.6% of harvest windows but only cost him anything on 24.5%; the gas utility's winter premium turns out to be Samuelson decay (15.5%) rather than seasonality (−0.5%); the quanto correlation's own instability is ~4x its price adjustment. Seventeen implementation and reporting bugs are documented, including four the Phase-2-only referee could never have caught. |
