...
| [020](src/research/020_basis_refinement_and_cross_venue.ipynb) | [Basis Refinement + Cross-Venue Dispersion](src/results/020_basis_refinement_and_cross_venue.md) | Tested a diversification floor + slower carry on 018's basis trade and a new Binance-Bybit funding-spread trade; the refined book clears net Sharpe (3.89) and deflated Sharpe cleanly but doesn't beat 018's own book on a paired basis, and the cross-venue spread underperforms the plain single-venue trade — holdout stays unspent on both. |
| [021](src/research/021_rc3_power_and_data_quality.ipynb) | [RC-3: Power or Data Quality?](src/results/021_rc3_power_and_data_quality.md) | Tested whether 020's non-firing paired-comparison gate was blocked by frozen-feed artifacts in the baseline's own return series or by sample size; a mechanical, pre-registered exclusion rule nominally clears the CI but fails its own placebo control, so the answer is statistical power — the comparison would need ~27.6 years of history against the 3.50 it has. |
