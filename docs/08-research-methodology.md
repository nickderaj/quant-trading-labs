...
**Important distinction: true ATR vs. approximation.** Notebook 11d uses a *true* ATR — Wilder's method applied to genuine OHLC data: $\mathrm{TR} = \max(\text{high} - \text{low}, |\text{high} - \text{prev\_close}|, |\text{low} - \text{prev\_close}|)$, then rolling mean over the window. Notebooks 11a–11c, working with calendar spreads (which have no OHLC), use an honest approximation: rolling standard deviation of day-over-day spread changes, also called "ATR" only because an external programme's config labeled it that way — two different methods solving the same problem (a volatility scale for position sizing) under different data constraints.
...
### Kullamagi-style breakout mechanism

**In one sentence.** A trend-following entry rule that requires a prior sustained uptrend, tightening consolidation range, breakout above the consolidation high, fixed ATR-based stop, moving-average trail exit, and a multi-symbol regime gate (fail-closed unless market trend signals confirm) — a single-entry-rule machine, not a sweep, with rigid parameters to avoid multiple-testing inflation on a thin historical sample.
