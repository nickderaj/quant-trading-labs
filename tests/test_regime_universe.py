...
def test_load_regime_universe_parses_macro_baskets_and_micro_complexes() -> None:
    universe = load_regime_universe(
        {
            "regime": {
                ...
                "macro_commodities": {"config": "commodity_default", "symbols": ["CL=F"]},
                "macro_fx": {"config": "fx_default", "symbols": ["6E=F"]},
                "micro": {
                    "oil_products": {"config": "commodity_default", "symbols": ["CL=F", "RB=F"]}
                },
            }
        }
    )
    ...
    assert [basket.name for basket in universe.baskets] == ["Commodities", "FX", "oil products"]
...
def test_load_regime_universe_matches_this_repos_configs_yaml() -> None:
    ...
    path = Path(__file__).resolve().parents[1] / "src" / "regime" / "configs" / "universe.yaml"
