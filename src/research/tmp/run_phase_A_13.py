...
def load_gc_spot_daily() -> pl.DataFrame:
    ...
    spot = spot.select(
        pl.col("timestamp").cast(pl.Date).alias("date"), pl.col("close").alias("spot_close")
    ).sort("date")
    return spot.with_columns(
        (pl.col("spot_close").log() - pl.col("spot_close").log().shift(1)).alias("spot_log_return")
    )
...
def build_position(
    ...
    vol_target_position = clipped_state * (VOL_TARGET / np.where(realized_vol > 1e-12, realized_vol, np.nan))

    if use_kelly:
        trailing_mean = pl.Series(log_ret).rolling_mean(window_size=20).shift(1).to_numpy()
        kelly_raw = E.fractional_kelly_scalar(trailing_mean, realized_vol, KELLY_FRACTION)
        kelly_multiplier = np.clip(np.abs(kelly_raw), 0.0, 1.0)
        kelly_multiplier = np.where(np.isfinite(kelly_multiplier), kelly_multiplier, 0.0)
    else:
    ...
    final_position_raw = np.where(np.isfinite(final_position_raw), final_position_raw, 0.0)
    return {
        "state": state,
...
def apply_stops_and_simulate(built: dict, optimistic_stop: bool = False) -> dict[str, np.ndarray]:
    ...
    n = len(close)
    atr_series = E.true_atr_series(high, low, close, window=14)  # already causal (shift(1))
    ...
    for t in range(1, n):
        ...
        if held_sign != 0:
            ...
            if np.isfinite(atr):
                if held_sign > 0:
                    ...
                    stop_level[t] = candidate if not np.isfinite(stop_level[t - 1]) else max(stop_level[t - 1], candidate)
                else:
                    candidate = close[t - 1] + ATR_MULT * atr
                    stop_level[t] = candidate if not np.isfinite(stop_level[t - 1]) else min(stop_level[t - 1], candidate)

            exit_mask, _ = E.apply_stop_fill(
                np.array([open_[t]]), np.array([high[t]]), np.array([low[t]]),
                np.array([stop_level[t]]), np.array([held_sign]), optimistic=optimistic_stop,
            )
            if exit_mask[0]:
        ...
        prev_sign = np.sign(realized_position[t]) if np.isfinite(realized_position[t]) else 0.0
        if prev_sign == 0:
            stop_level[t] = np.nan
...
def bar_returns_with_costs(close: np.ndarray, position: np.ndarray) -> dict[str, np.ndarray]:
    log_ret = np.diff(np.log(close), prepend=np.log(close[0]))
    log_ret[0] = 0.0
    gross = position[:-1] * log_ret[1:]  # position held through bar t-1 earns bar t's return
...
def run_trial(df: pl.DataFrame, name: str, use_smoothing=True, use_impact=True, use_kelly=True, optimistic_stop=False):
    ...
    for offset in ORIGIN_OFFSETS:
        splits_o = research.walk_forward_splits(n, TRAIN_BARS, TEST_BARS, mode="rolling", origin_offset=offset)
        idx_o = np.unique(np.concatenate([test for _, test in splits_o])) if splits_o else np.array([], dtype=int)
        net_o = rc["net"][idx_o]
    ...
    dsr = research.deflated_sharpe_prob(
        by_offset["offset_0"]["sharpe"] / ANNUALIZED_RATE, n_trials=N_TRIALS_POOLED, n_obs=by_offset["offset_0"]["no_bars"]
    )

    return {
        "name": name,
        "n_oos_bars": int(len(oos_idx)),
        "oos_date_range": [str(df["date"][int(oos_idx.min())]), str(df["date"][int(oos_idx.max())])],
...
def benchmark_neutrality(dates: list, net_returns: np.ndarray, spot: pl.DataFrame) -> dict:
    df = pl.DataFrame({"date": dates, "strat": net_returns}).join(spot, on="date", how="inner").drop_nulls()
    if df.height < 30:
        return {"beta": None, "alpha_annualized": None, "information_ratio": None, "n_obs": df.height}
    y = df["strat"].to_numpy()
    x = df["spot_log_return"].to_numpy()
    beta, alpha = np.polyfit(x, y, 1)
    resid = y - (alpha + beta * x)
    ir = float(np.mean(resid) / np.std(resid) * ANNUALIZED_RATE) if np.std(resid) > 0 else 0.0
    return {
        "beta": float(beta),
...
def main():
    ...
    capacity = E.capacity_curve(
        base_notional=float(np.nanmean(np.abs(sim_base["realized_position"][base["oos_idx"]])) * ASSUMED_AUM),
    ...
    rc_optimistic = bar_returns_with_costs(built_base["close"], sim_optimistic["realized_position"])
    optimistic_metrics = series_metrics(rc_optimistic["net"][base["oos_idx"]], "base_optimistic_stop_fill")

    cost_sens = {}
    gross_ret = np.diff(np.log(built_base["close"]), prepend=np.log(built_base["close"][0]))
    gross_ret[0] = np.nan
    ...
    net_sens = gross_pnl + np.log(np.clip(1 - (sensitivity_cost_bps / 1e4) * turnover, 1e-6, None))
    ...
    out = {
        ...
        "gate_FF_fires": bool(
            ...
            and len({np.sign(trials["base"]["by_offset"][f"offset_{o}"]["sharpe"]) for o in ORIGIN_OFFSETS}) == 1
        ),
    }

    with open("src/research/tmp/phase_A_13_results.json", "w") as f:
        json.dump(out, f, indent=2, default=str)
    print(json.dumps({k: v for k, v in out.items() if k != "trials"}, indent=2, default=str)[:3000])
    print("\nbase net sharpe:", trials["base"]["net"]["sharpe"], "n_oos_bars:", trials["base"]["n_oos_bars"])
