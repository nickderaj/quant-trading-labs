...
RESULTS = _load("src/research/tmp/backtest_results.json")
...
print(
    f"cfg2_12h offset=0 sharpe_net={sharpe_net_0:.3f} (write-up: +0.42), "
    f"sharpe_gross={sharpe_gross_0:.3f} (write-up: +1.32)"
)
assert abs(sharpe_net_0 - 0.42) < 0.005, (
    f"cfg2_12h offset0 net Sharpe {sharpe_net_0} != write-up's +0.42"
)
assert abs(sharpe_gross_0 - 1.32) < 0.005, (
    f"cfg2_12h offset0 gross Sharpe {sharpe_gross_0} != write-up's +1.32"
)
...
assert abs(sharpe_net_7 - (-2.45)) < 0.005, (
    f"cfg2_12h offset7 net Sharpe {sharpe_net_7} != write-up's -2.45"
)

# ---- 3. cfg2_12h realized turnover/fee drag: ~0.33-0.37%/yr annual fee drag ----
fee_drags = [
    cfg2[str(o)]["stitched_metrics"]["annual_fee_drag_pct"] for o in (0, 7, 14, 21)
]
print(
    f"cfg2_12h annual_fee_drag_pct across offsets: {[f'{x:.3f}' for x in fee_drags]} "
...
for cfg_id in ("cfg1_4h", "cfg3_1d"):
    ...
    print(
        f"{cfg_id} sharpe_net across offsets: {[f'{x:.3f}' for x in sharpes]} (write-up: all negative)"
    )
    assert all(x < 0 for x in sharpes), (
        f"{cfg_id} expected negative net Sharpe at every offset, got {sharpes}"
    )

# ---- 5. holdout Sharpe (Phase 7, spent once): net -0.47, gross +0.74 ----
HOLDOUT = _load("src/research/tmp/holdout_results.json")
holdout_net = HOLDOUT["holdout_metrics"]["sharpe_net"]
holdout_gross = HOLDOUT["holdout_metrics"]["sharpe"]
print(
    f"holdout sharpe_net={holdout_net:.3f} (write-up: -0.47), "
    f"sharpe_gross={holdout_gross:.3f} (write-up: +0.74)"
)
assert abs(holdout_net - (-0.47)) < 0.005, (
    f"holdout net Sharpe {holdout_net} != write-up's -0.47"
)
assert abs(holdout_gross - 0.74) < 0.005, (
    f"holdout gross Sharpe {holdout_gross} != write-up's +0.74"
)
