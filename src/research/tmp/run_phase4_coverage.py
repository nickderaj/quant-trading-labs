...
for interval in INTERVALS:
    ...
    gpd_paths_d8, _gpd_fits_d8 = L5.rolling_gpd_paths(ret, fits_d4, model="garch", max_train=MLE_MAX_TRAIN, tail_frac=0.10)
    gpd_paths_d9, _gpd_fits_d9 = L5.rolling_gpd_paths(ret, fits_d6, model="gjr", max_train=MLE_MAX_TRAIN, tail_frac=0.10)
    ...
    for name in ["d0_trailing_std", "d1_har_rv", "d2_har_log_rv", "d3_range", "d4_garch_normal", "d6_gjr_normal"]:
        ...
        es_forecasts[name] = {q: L5.normal_es_forecast(variance_fc[name], q) for q in QUANTILES}
    quantile_forecasts["d5_garch_t"] = L5.t_quantile_forecasts(variance_fc["d5_garch_t"], nu_d5, QUANTILES)
    es_forecasts["d5_garch_t"] = {q: L5.t_es_forecast(variance_fc["d5_garch_t"], nu_d5, q) for q in QUANTILES}
    quantile_forecasts["d7_gjr_t"] = L5.t_quantile_forecasts(variance_fc["d7_gjr_t"], nu_d7, QUANTILES)
    es_forecasts["d7_gjr_t"] = {q: L5.t_es_forecast(variance_fc["d7_gjr_t"], nu_d7, q) for q in QUANTILES}
    quantile_forecasts["d8_garch_evt"] = L5.gpd_quantile_forecasts(variance_fc["d4_garch_normal"], gpd_paths_d8, QUANTILES)
    es_forecasts["d8_garch_evt"] = {q: L5.gpd_es_forecast(variance_fc["d4_garch_normal"], gpd_paths_d8, q) for q in QUANTILES}
    quantile_forecasts["d9_gjr_evt"] = L5.gpd_quantile_forecasts(variance_fc["d6_gjr_normal"], gpd_paths_d9, QUANTILES)
    es_forecasts["d9_gjr_evt"] = {q: L5.gpd_es_forecast(variance_fc["d6_gjr_normal"], gpd_paths_d9, q) for q in QUANTILES}
    ...
    for name, qf in quantile_forecasts.items():
        ...
        never_rejected = all(
            (level["kupiec_p"] > 0.05) and (level["indep_p"] > 0.05) and (level["cc_p"] > 0.05)
            for level in battery.values()
        )
        underpowered_levels = [q for q, level in battery.items() if level["n_violations"] < 10]
    ...
    underlying_variance_col = {
        "d8_garch_evt": "d4_garch_normal", "d9_gjr_evt": "d6_gjr_normal",
    }
    sigma_by_model = {
        name: np.sqrt(np.where(
            variance_fc[underlying_variance_col.get(name, name)] > 0,
            variance_fc[underlying_variance_col.get(name, name)], np.nan,
        ))
    ...
    for name in quantile_forecasts:
        ...
        for q in [0.01, 0.05]:
            ...
            p_boot = L5.acerbi_szekely_bootstrap_pvalue(z_obs, sim_fn, n_boot=ES_BOOTSTRAP_N, seed=0)
            es_backtest[name][str(q)] = {"z": z_obs, "bootstrap_pvalue": p_boot}

    res_out = {
        "n_obs": n, "coverage": coverage, "gate_b_verdict": gate_b, "es_backtest": es_backtest,
...
n_any_gate_b_interval = sum(1 for iv in out["intervals"].values() if any(iv["gate_b_verdict"].values()))
out["gate_b_summary"] = {"n_intervals_with_any_clearer": n_any_gate_b_interval, "n_intervals": len(INTERVALS)}
print(f"Gate B: {n_any_gate_b_interval}/{len(INTERVALS)} intervals have at least one model clearing all 36 tests")
