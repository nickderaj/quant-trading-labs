...
N_TRIALS_SPR_BW = 1  # the brent_wti both-legs-agree variant -- from phase_5_10a_results.json
...
def load_regime_frame(product: str) -> pl.DataFrame:
    ...
    return ts.select(["date", "term_structure_state", "state_deadband", "state_persistent"])


def spread_returns_gated(name: str, leg_products: list[str], regime_col: str, regime_frame: pl.DataFrame) -> pl.DataFrame | None:
    ...
    no_trade = pl.col(regime_col).is_in(["flat", "unconfirmed"]) | pl.col(regime_col).is_null()
    ...
    capital_basis = np.array([sum(abs(row[i]) * point_values[i] for i in range(n_legs)) for row in leg_prices])
...
def build_book(returns_by_spread: dict[str, pl.DataFrame], origin_offset: int) -> pl.DataFrame:
    per_spread = []
    ...
    book = book.with_columns(pl.mean_horizontal([pl.col(c) for c in return_cols]).alias("book_net_return")).sort("date")
...
def main():
    ...
    regime_frames = {name: load_regime_frame(taxonomy[name]["leg_products"][0]) for name in eligible_names}

    definitions = {"raw_sign": "term_structure_state", "deadband": "state_deadband", "persistent": "state_persistent"}
    ...
    unconditional_returns = {
        name: spread_daily_returns(name, taxonomy[name]["leg_products"]) for name in eligible_names
    }

    def per_offset_metrics(returns_dict: dict, offset: int) -> tuple[dict, pl.DataFrame]:
    ...
    for def_name in definitions:
        ...
        exceeds_every_offset = all(
            np.isfinite(sg) and np.isfinite(su) and sg > su for sg, su in zip(sharpes_g, sharpes_u)
        )

        headline_g = gated_books[0].join(unconditional_books[0], on="date", how="inner", suffix="_u")
        diff = (headline_g["book_net_return"] - headline_g["book_net_return_u"]).to_numpy()
        diff = diff[np.isfinite(diff)]
        diff_ci = research.block_bootstrap_ci(diff, n_boot=2000, seed=0) if len(diff) > 30 else (None, None)
        diff_ci_excludes_zero = diff_ci[0] is not None and (diff_ci[0] > 0 or diff_ci[1] < 0)

        gated_headline_returns = gated_books[0]["book_net_return"].to_numpy()
        ci_zero = research.block_bootstrap_ci(gated_headline_returns, n_boot=2000, seed=0) if len(gated_headline_returns) > 30 else (None, None)
        ...
        dsr = (
            research.deflated_sharpe_prob(by_offset_gated["offset_0"]["sharpe"] / ANNUALIZED_RATE, n_trials=n_trials, n_obs=by_offset_gated["offset_0"]["n"])
            if n_trials and np.isfinite(by_offset_gated["offset_0"]["sharpe"])
            else None
        )

        sp_full_criterion_met = bool(gated_all_positive and ci_zero_excludes and dsr is not None and dsr > 0.95)
        spr_by_definition[def_name] = {
            ...
            "fires": bool(def_name == "deadband" and sp_full_criterion_met and exceeds_every_offset and diff_ci_excludes_zero),
        }
    ...
    for name in eligible_names:
        ...
        if g is None or u is None:
            per_spread_bw[name] = {"gated_sharpe": None, "unconditional_sharpe": None, "gated_exceeds": None}
            continue
        mg, mu = series_metrics(g["net_return"].to_numpy()), series_metrics(u["net_return"].to_numpy())
        per_spread_bw[name] = {
            "gated_sharpe": mg["sharpe"],
            "unconditional_sharpe": mu["sharpe"],
            "gated_exceeds": bool(np.isfinite(mg["sharpe"]) and np.isfinite(mu["sharpe"]) and mg["sharpe"] > mu["sharpe"]),
        }
    ...
    bz_regime = load_regime_frame("BZ").rename({c: f"bz_{c}" for c in ["term_structure_state", "state_deadband", "state_persistent"]})
    cl_regime = load_regime_frame("CL").rename({c: f"cl_{c}" for c in ["term_structure_state", "state_deadband", "state_persistent"]})
    ...
    both_regime_for_gate = both_regime.select(["date", "both_agree_deadband"]).rename({"both_agree_deadband": "state_for_gate"})
    both_regime_for_gate = both_regime_for_gate.with_columns(
        pl.when(pl.col("state_for_gate").is_in(["flat", "disagree"])).then(pl.lit("flat")).otherwise(pl.col("state_for_gate")).alias("state_for_gate")
    )
    bw_both_agree_returns = spread_returns_gated("brent_wti", taxonomy["brent_wti"]["leg_products"], "state_for_gate", both_regime_for_gate)
    bw_unconditional_returns = unconditional_returns["brent_wti"]
    m_bw_gated = series_metrics(bw_both_agree_returns["net_return"].to_numpy()) if bw_both_agree_returns is not None else {"sharpe": None}
    m_bw_unconditional = series_metrics(bw_unconditional_returns["net_return"].to_numpy()) if bw_unconditional_returns is not None else {"sharpe": None}
        ...
        "gate_SPR_bw": {
            ...
            "brent_wti_both_legs_agree_secondary_variant": {
                ...
                "gated_exceeds_unconditional": bool(
                    np.isfinite(m_bw_gated["sharpe"]) and np.isfinite(m_bw_unconditional["sharpe"])
                    and m_bw_gated["sharpe"] > m_bw_unconditional["sharpe"]
                ) if m_bw_gated["sharpe"] is not None and m_bw_unconditional["sharpe"] is not None else None,
            },
        },
    }
    ...
    for def_name, r in spr_by_definition.items():
        print(f"SPR[{def_name}]: fires={r['fires']} dsr={r['deflated_sharpe_prob']} "
              f"exceeds_every_offset={r['gated_exceeds_unconditional_every_offset']}")
    print(f"SPR-BW: fires={gate_spr_bw_fires} brent_wti_exceeds={brent_wti_exceeds} others={others_exceeding}/{len(eligible_names)-1}")
    print(f"brent_wti both-legs-agree: gated_sharpe={m_bw_gated['sharpe']} unconditional_sharpe={m_bw_unconditional['sharpe']}")
