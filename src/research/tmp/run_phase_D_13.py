...
N_TRIALS_POOLED = 18
DEV_START = datetime(2021, 7, 1, tzinfo=timezone.utc)
...
def restrict_to_complete_symbols(panel: pl.DataFrame) -> pl.DataFrame:
...
def build_features(panel: pl.DataFrame) -> pl.DataFrame:
    ...
    for lag in range(1, 6):
        panel = panel.with_columns(pl.col("ret").shift(lag).over("symbol").alias(f"lag_{lag}"))
...
def wide_matrices(panel: pl.DataFrame, symbols: list[str], feature_cols: list[str]):
    ...
    for col in feature_cols + ["fwd_ret"]:
        wide[col] = panel.pivot(index="datetime", on="symbol", values=col).sort("datetime").select(["datetime"] + symbols)
    dates = wide[feature_cols[0]]["datetime"].to_list()
    return dates, wide


def train_and_predict(
    panel: pl.DataFrame, symbols: list[str], feature_cols: list[str], use_time_mixing: bool, seed: int = 0
) -> pl.DataFrame:
    ...
    model = E.GraphAttentionPredictor(n_features, hidden_dim=HIDDEN_DIM, use_time_mixing=use_time_mixing, time_window=5)
    ...
    for train_idx, test_idx in splits:
        for epoch in range(N_EPOCHS_PER_FOLD):
            ...
            if date not in graphs:
                continue
            x, y, adj = build_step_tensors(wide, feature_cols, symbols, t, use_time_mixing)
            if x is None:
                continue
            optimizer.zero_grad()
            pred = model(x, adj)
        ...
        with torch.no_grad():
            for t in test_idx:
                ...
                if date not in graphs:
                    continue
                x, y, adj = build_step_tensors(wide, feature_cols, symbols, int(t), use_time_mixing)
                if x is None:
                    continue
                pred = model(x, adj).numpy()
                for i, sym in enumerate(symbols):
                    rows.append({"datetime": date, "symbol": sym, "pred": float(pred[i]), "fwd_ret": float(y[i]) if np.isfinite(y[i]) else None})
        model.train()
...
def rolling_graph(panel: pl.DataFrame, symbols: list[str]) -> dict:
    ...
    return E.rolling_causal_corr_graph(ret_panel, lookback=CORR_LOOKBACK, threshold=CORR_THRESHOLD, return_col="ret")
...
def build_step_tensors(wide, feature_cols, symbols, t, use_time_mixing):
    ...
    y = torch.tensor([v if v is not None else float("nan") for v in y_row], dtype=torch.float32)
    ...
    if use_time_mixing:
        ...
        x = torch.tensor(np.stack(feats, axis=1), dtype=torch.float32)  # (n_symbols, window, n_features)
    else:
        row = []
        for fc in feature_cols:
            vals = wide[fc].row(t)[1:]
            row.append([v if v is not None else 0.0 for v in vals])
        x = torch.tensor(np.array(row).T, dtype=torch.float32)  # (n_symbols, n_features)
    return x, y, adj


def gate_ic(preds: pl.DataFrame) -> dict:
    ic_stats = research.cross_sectional_ic_stats(
        research.cross_sectional_ic(preds, pred_col="pred", target_col="fwd_ret", datetime_col="datetime"),
        nw_lag=5,
    )
    passes_003_filter = bool(abs(ic_stats.get("nw_tstat", 0)) > 3)
    leak_flag = bool(abs(ic_stats.get("mean_ic", 0)) > 0.10)
    return {**ic_stats, "passes_003_survival_filter": passes_003_filter, "leak_suspected": leak_flag}
...
def build_books(preds: pl.DataFrame) -> dict:
    ...
    dn = research.dollar_neutral_weights(preds, pred_col="pred", datetime_col="datetime", top_frac=TOP_K / preds["symbol"].n_unique())
    return {"long_only": long_only, "dollar_neutral": dn}


def book_metrics(weights: pl.DataFrame, returns: pl.DataFrame, label: str) -> dict:
    trade_frame = research.portfolio_trade_frame(weights, returns, target_col="fwd_ret", datetime_col="datetime")
    ...
    joined = joined.with_columns(
        (pl.col("trade_log_return") - (TAKER_FEE + SLIPPAGE) * pl.col("turnover")).alias("trade_log_return_net")
    )
    ...
    def m(x, lbl):
        ...
        return {"label": lbl, "sharpe": (mean / std) * ANNUALIZED_RATE if std > 0 else 0.0, "n_bars": len(x)}

    ci_lo, ci_hi = research.block_bootstrap_ci(netr, seed=0) if len(netr) > 30 else (None, None)
    return {
...
def main():
    ...
    preds_no_mix = train_and_predict(panel, symbols, feature_cols, use_time_mixing=False)
    ...
    ic_diff_series = (
        preds_no_mix.select("datetime", "symbol", pl.col("pred").alias("pred_no_mix"))
        .join(preds_mix.select("datetime", "symbol", pl.col("pred").alias("pred_mix"), "fwd_ret"), on=["datetime", "symbol"])
    )
    ic_no_mix_by_date = research.cross_sectional_ic(ic_diff_series.rename({"pred_no_mix": "pred"}), "pred", "fwd_ret", "datetime")
    ic_mix_by_date = research.cross_sectional_ic(ic_diff_series.rename({"pred_mix": "pred"}), "pred", "fwd_ret", "datetime")
    joined_ic = ic_no_mix_by_date.join(ic_mix_by_date, on="datetime", suffix="_mix")
    ic_col = [c for c in joined_ic.columns if c not in ("datetime",) and not c.endswith("_mix")][0]
    ic_col_mix = ic_col + "_mix"
    diff = (joined_ic[ic_col] - joined_ic[ic_col_mix]).drop_nulls().to_numpy()
    tm_ci = research.block_bootstrap_ci(diff, seed=0) if len(diff) > 30 else (None, None)
    ...
    dn_metrics = book_metrics(books_no_mix["dollar_neutral"], returns_no_mix, "dollar_neutral")

    basket = research.equal_weight_basket_returns(returns_no_mix, target_col="fwd_ret", datetime_col="datetime")
    dn_trade = research.portfolio_trade_frame(books_no_mix["dollar_neutral"], returns_no_mix, target_col="fwd_ret", datetime_col="datetime")
    joined_beta = dn_trade.join(basket.rename({"trade_log_return": "basket_return"}), on="datetime", how="inner").drop_nulls()
    if joined_beta.height > 30:
        beta = float(np.polyfit(joined_beta["basket_return"].to_numpy(), joined_beta["trade_log_return"].to_numpy(), 1)[0])
    else:
        beta = None

    throttled = A.throttle_weights(books_no_mix["dollar_neutral"], k=3)
    throttled_metrics = book_metrics(throttled, returns_no_mix, "dollar_neutral_throttled")

    dsr_dn = research.deflated_sharpe_prob(dn_metrics["net"]["sharpe"] / ANNUALIZED_RATE, N_TRIALS_POOLED, dn_metrics["net"]["n_bars"]) if dn_metrics["net"]["n_bars"] > 1 else float("nan")
    ...
    gate_xs_fires = bool(
        ...
        and beta is not None and abs(beta) < 0.2
    )
    ...
    out = {
        ...
        "ic_003_context": {"mean_reversion_ic": 0.042, "realized_vol_ic": -0.038, "source": "notebook 003"},
