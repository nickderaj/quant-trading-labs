...
def run_one_symbol(symbol: str) -> dict:
    ...
    gpd_paths_d8, _ = L5.rolling_gpd_paths(ret, fits_d4, model="garch", max_train=MLE_MAX_TRAIN, tail_frac=0.10)
    gpd_paths_d9, _ = L5.rolling_gpd_paths(ret, fits_d6, model="gjr", max_train=MLE_MAX_TRAIN, tail_frac=0.10)
    ...
    frac_leverage_normal = float(np.mean([p < 0.05 for p in gjr_normal_lr])) if gjr_normal_lr else None
    ...
    gate_a_models = [name for name in variance_fc if name not in ("d8_garch_evt", "d9_gjr_evt")]
    ...
    for a, b in itertools.combinations(gate_a_models, 2):
        ...
        key = f"{a}_vs_{b}"
        all_pairs_dm[key] = {"a": a, "b": b, "tstat": tstat, "normal_pvalue": p_normal, "bootstrap_pvalue": p_boot}
    ...
    for other in gate_a_models:
        ...
        best_wins_sig = bh["significant"] and ((a_is_best and entry["tstat"] < 0) or (not a_is_best and entry["tstat"] > 0))
    ...
    for name in ["d0_trailing_std", "d1_har_rv", "d2_har_log_rv", "d3_range", "d4_garch_normal", "d6_gjr_normal"]:
        quantile_forecasts[name] = L5.normal_quantile_forecasts(variance_fc[name], QUANTILES)
    quantile_forecasts["d5_garch_t"] = L5.t_quantile_forecasts(variance_fc["d5_garch_t"], nu_paths["d5_garch_t"], QUANTILES)
    quantile_forecasts["d7_gjr_t"] = L5.t_quantile_forecasts(variance_fc["d7_gjr_t"], nu_paths["d7_gjr_t"], QUANTILES)
    quantile_forecasts["d8_garch_evt"] = L5.gpd_quantile_forecasts(variance_fc["d4_garch_normal"], gpd_paths_d8, QUANTILES)
    quantile_forecasts["d9_gjr_evt"] = L5.gpd_quantile_forecasts(variance_fc["d6_gjr_normal"], gpd_paths_d9, QUANTILES)

    gate_b_by_model = {}
    for name, qf in quantile_forecasts.items():
        battery = L5.coverage_battery(ret, qf)
        gate_b_by_model[name] = all(
            (lvl["kupiec_p"] > 0.05) and (lvl["indep_p"] > 0.05) and (lvl["cc_p"] > 0.05)
            for lvl in battery.values()
        )
    gate_b_any = any(gate_b_by_model.values())

    return {
        "n_obs": n, "scores": scores, "best_by_log_score": best_name, "gate_a_fires": gate_a_fires,
        "gate_b_by_model": gate_b_by_model, "gate_b_any_clears": gate_b_any,
        "gjr_leverage": {"frac_refits_significant_gamma0_normal": frac_leverage_normal,
...
for symbol in TRANSFER_SYMBOLS:
    ...
    print(f"{symbol}: n={res['n_obs']} elapsed={res['elapsed_sec']:.1f}s "
...
btc_density = json.load(open("src/research/tmp/phase3_density_results.json"))["intervals"]["1d"]
btc_coverage = json.load(open("src/research/tmp/phase4_coverage_results.json"))["intervals"]["1d"]
btc_gate_a = btc_density["gate_a_verdict"]["beats_every_other_significantly_bootstrap_bh"]
btc_gate_b = btc_coverage["gate_b_summary"] if "gate_b_summary" in btc_coverage else None
btc_best = btc_density["gate_a_verdict"]["best_by_log_score"]

all_symbols_gate_a = [btc_gate_a] + [out["symbols"][s]["gate_a_fires"] for s in TRANSFER_SYMBOLS]
n_gate_a_fire = sum(all_symbols_gate_a)
out["gate_c_verdict"] = {
    "btc_1d_best_model": btc_best, "btc_1d_gate_a_fires": btc_gate_a,
    "n_of_6_symbols_gate_a_fires": n_gate_a_fire,
    "stable_no_winner_pattern": n_gate_a_fire <= 1,  # BTC didn't fire; check if transfer mostly agrees
    "transfer_best_models": {s: out["symbols"][s]["best_by_log_score"] for s in TRANSFER_SYMBOLS},
}
print(f"Gate C: {n_gate_a_fire}/6 symbols (incl. BTC) have a significant Gate A winner at 1d; "
