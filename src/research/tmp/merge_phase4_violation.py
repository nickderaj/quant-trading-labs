...
for symbol in SYMBOLS:
    d = json.load(open(f"src/research/tmp/phase4_violation_{symbol}.json"))
    for interval in INTERVALS:
        models = d["intervals"][interval]["models"]
        for model_name, cell in models.items():
            counts_sig = bool(cell["counts"]["nb_significantly_better"]) if cell["counts"] else None
            dur_sig = bool(cell["durations"]["weibull_significantly_better"]) if cell["durations"] else None
            dur_clusters = bool(cell["durations"]["clusters"]) if cell["durations"] else None
            rejects_null = bool(counts_sig) or bool(dur_sig)
            cells.append({
                "symbol": symbol, "interval": interval, "model": model_name,
                "counts_significant": counts_sig, "durations_significant": dur_sig,
...
print(f"durations (discrete-Weibull vs geometric) significant: {n_dur_sig}/{n_dur_valid} "
      f"(of which beta<1 / clustering: {n_dur_cluster})")
print(f"cells rejecting i.i.d.-Bernoulli null (counts OR durations): {n_reject}/{n_total} "
...
by_model = defaultdict(lambda: {"n": 0, "n_reject": 0})
for c in cells:
...
out = {
    "n_total_cells": n_total, "n_counts_significant": n_counts_sig, "n_counts_valid": n_counts_valid,
    "n_durations_significant": n_dur_sig, "n_durations_valid": n_dur_valid,
    "n_durations_clustering_beta_lt_1": n_dur_cluster,
    "n_reject_null": n_reject, "frac_reject_null": n_reject / n_total,
