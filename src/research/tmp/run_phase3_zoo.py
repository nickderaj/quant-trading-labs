...
from densities import REGISTRY  # noqa: E402
...
def run_one_interval(symbol: str, interval: str) -> dict:
    ...
    log_score_full["garch_normal"] = L5.vectorized_normal_scores(ret, variance_fc["garch_normal"])["log_score"]
    mask_n = np.isfinite(ret) & np.isfinite(variance_fc["garch_normal"]) & (variance_fc["garch_normal"] > 0)
    ...
    variance_fc["garch_t"], fits_t = L.rolling_garch_forecast(
        ret, refit_every=mle_refit_every, min_train=min_train, innovation="t", max_train=L6.MLE_MAX_TRAIN,
    )
    ...
    for fam_name in ZOO_FAMILIES:
        ...
        fc, fits = L6.rolling_garch_forecast_zoo(
            ret, refit_every=mle_refit_every, min_train=min_train, family_module=family_module,
    ...
    scores = {
        name: {"log_score_mean": float(np.nanmean(log_score_full[name])),
    ...
    for fam_key in zoo_only:
        key = f"{fam_key}_vs_garch_t" if f"{fam_key}_vs_garch_t" in all_pairs_dm else f"garch_t_vs_{fam_key}"
        entry = all_pairs_dm.get(key)
        beats_garch_t = False
        significant = False
        if entry is not None:
            a_is_fam = entry["a"] == fam_key
            fam_wins = (a_is_fam and entry["tstat"] < 0) or (not a_is_fam and entry["tstat"] > 0)
            significant = entry["bh_bootstrap"]["significant"]
            beats_garch_t = fam_wins and scores[fam_key]["log_score_mean"] > scores["garch_t"]["log_score_mean"]
        gate_p_per_family[fam_key] = {
            "beats_garch_t_on_mean": scores[fam_key]["log_score_mean"] > scores["garch_t"]["log_score_mean"],
            "significantly_beats_garch_t": beats_garch_t and significant,
        }

    return {
        "n_obs": n, "scores": scores, "all_pairs_dm": all_pairs_dm,
        "best_by_log_score": max(model_names, key=lambda name: scores[name]["log_score_mean"]),
        "best_zoo_family": best_zoo, "gate_p_per_family": gate_p_per_family,
        "shape_paths": shape_paths, "elapsed_sec": time.time() - t0,
    }
