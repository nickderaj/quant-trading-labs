...
data = {s: _load(f"src/research/tmp/phase5_evt_density_{s}.json") for s in SYMBOLS}

out: dict[str, Any] = {
    "intervals": {},
    "scope_note": "1h dropped (compute budget, see module docstring)",
}
for interval in INTERVALS:
    ...
    for s in SYMBOLS:
        ...
        def _evt_beats_all_non_evt(
            evt_name, all_pairs_dm=iv["all_pairs_dm"], non_evt_names=non_evt_names
        ):
            for other in non_evt_names:
                key = (
                    f"{evt_name}_vs_{other}"
                    if f"{evt_name}_vs_{other}" in all_pairs_dm
                    else f"{other}_vs_{evt_name}"
                )
                ...
                wins_sig = bh["significant"] and (
                    (a_is_evt and entry["tstat"] < 0)
                    or (not a_is_evt and entry["tstat"] > 0)
                )
                if not wins_sig:
                    return False
            return True
    ...
    print(
        f"  EVT significantly dominates every non-EVT model on {n_evt_dominates}/6 symbols "
        f"(BTC: {btc_dominates})"
    )
    print(
        f"  cross-sectional EVT-wins-Gate-A verdict (>=5/6 incl. BTC): "
        f"{out['intervals'][interval]['evt_wins_gate_a_cross_sectionally']}"
    )
    for s in SYMBOLS:
        h8, h9 = per_symbol[s]["d8_health"], per_symbol[s]["d9_health"]
        print(
            f"    {s}: best={per_symbol[s]['best']} "
