...
data = {s: json.load(open(f"src/research/tmp/phase3_zoo_{s}.json")) for s in SYMBOLS}
...
for interval in INTERVALS:
    ...
    for fam in ZOO_FAMILIES:
        n_sig = sum(1 for s in SYMBOLS if per_symbol[s]["gate_p_per_family"][fam]["significantly_beats_garch_t"])
        btc_sig = per_symbol["BTCUSDT"]["gate_p_per_family"][fam]["significantly_beats_garch_t"]
        n_beats_mean = sum(1 for s in SYMBOLS if per_symbol[s]["gate_p_per_family"][fam]["beats_garch_t_on_mean"])
        gate_p_verdict[fam] = {
    ...
    out["intervals"][interval] = {"per_symbol": per_symbol, "gate_p_verdict": gate_p_verdict}
    print(f"--- {interval} ---")
    for fam, v in gate_p_verdict.items():
        print(f"  {fam}: sig on {v['n_significantly_beats_garch_t_of_6']}/6 (BTC sig={v['btc_significant']}), "
...
for interval in INTERVALS:
    ...
    for fam in ZOO_FAMILIES:
        ...
        shape_summary[interval][fam] = {
            ...
            "median": arr.mean(axis=0).tolist() if arr.ndim > 1 else [float(arr.mean())],
            "per_param_median": [float(np.median(arr[:, i])) for i in range(arr.shape[1])],
            "per_param_range": [[float(arr[:, i].min()), float(arr[:, i].max())] for i in range(arr.shape[1])],
        }
