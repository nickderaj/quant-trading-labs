...
ORIGIN_OFFSETS = [
    ...
    14 * 4,
    21 * 4,
]  # offsets in bars at 6h cadence (approx days*4)
N_TRIALS_POOLED = 18
TAKER_FEE = 0.0004
SLIPPAGE = 0.0001
...
def score_month_config(
    ...
    for sym, g in month_df.group_by("symbol"):
        ...
        _exit_mask, _fill = E.apply_stop_fill(
    ...
    for sym, g in df.group_by("symbol"):
        ...
        if use_stop:
            atr_mult = (
                ...
                else (params_by_month[2] if isinstance(params_by_month, tuple) else 3.0)
            )
            stop = E.trailing_atr_stop(close, high, low, entry, atr_mult=atr_mult)
            open_ = np.roll(close, 1)
            exit_mask, _fill_price = E.apply_stop_fill(
        ...
        if use_selection:
            eligible_long = E.quality_liquidity_selection(
                tsharpe,
                dvol,
                LONG_QUALITY,
                SHORT_QUALITY,
                LIQUIDITY_RANK_FRAC if use_liquidity else 1.0,
                "long",
            )
            eligible_short = E.quality_liquidity_selection(
                tsharpe,
                dvol,
                LONG_QUALITY,
                SHORT_QUALITY,
            ...
            rank = (
                pl.Series(dvol).rank(method="average", descending=True).to_numpy() / n
            )
...
def book_returns(sim: pl.DataFrame) -> pl.DataFrame:
    def leg_return(df: pl.DataFrame, sign: int, capital_frac: float) -> pl.DataFrame:
        ...
        leg = leg.with_columns(
            (pl.col("ret") * sign * capital_frac / pl.col("n")).alias("weighted_ret")
        )
...
def main():
    ...
    trials["ablation_no_selection"] = evaluate(
        panel, adaptive_params, use_selection=False
    )
    print("Evaluating: ablation no_liquidity...", flush=True)
    trials["ablation_no_liquidity"] = evaluate(
        panel, adaptive_params, use_liquidity=False
    )
    ...
    trials["adaptive_excl_delisted"] = evaluate(
        panel, adaptive_params, exclude_delisted=True
    )
    ...
    for off in ORIGIN_OFFSETS:
        by_offset[f"offset_{off}"] = evaluate(
            panel, adaptive_params, origin_offset_bars=off
        )

    adaptive_net = trials["adaptive"]["net"]["sharpe"]
    _frozen_net = trials["frozen_twin"]["net"]["sharpe"]
    ...
    ablation_predicted_direction = {
        ...
        "no_liquidity": bool(
            trials["ablation_no_liquidity"]["net"]["sharpe"] < adaptive_net
        ),
        "no_adaptation": bool(
    ...
    gate_at_fires = bool(
        ...
        and adaptive_net
        > 1.5  # sec3 bar floor context; full bar also needs null-beat, checked in writeup
