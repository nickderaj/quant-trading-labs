...
PHASE3 = _load("src/research/tmp/phase3_density_results.json")
PHASE4 = _load("src/research/tmp/phase4_coverage_results.json")
...
assert abs(garch_t_12h - 2.623) < 0.001, (
    f"GARCH-t 12h log score {garch_t_12h} does not match write-up's 2.623"
)
...
assert len(clearers_12h) == 1, (
    f"expected exactly 1 clearer at 12h, got {len(clearers_12h)}: {clearers_12h}"
)
assert clearers_12h[0] == "d8_garch_evt", (
    f"expected GARCH-EVT (d8) to be the sole 12h clearer, got {clearers_12h[0]}"
)

# ---- 3. the Gate A verdict at 1d (no significant winner) ----
gate_a_1d = PHASE3["intervals"]["1d"]["gate_a_verdict"]
print(
    f"1d Gate A verdict: best={gate_a_1d['best_by_log_score']} "
