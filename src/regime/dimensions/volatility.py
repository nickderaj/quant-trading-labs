...
def atr_percentile(inputs: RegimeInputs, window: int = 14, pct_window: int = 252) -> pd.Series:
...
def realized_vol_percentile(
    ...
    return 2 * rolling_percentile_rank(realized_vol(inputs.ohlcv["close"], window), pct_window) - 1
...
def bollinger_width_percentile(
    ...
    return (
        2 * rolling_percentile_rank(bollinger_width(inputs.ohlcv["close"], window), pct_window) - 1
    )


@register("vol.vix_level", {"ohlcv", "macro"}, column="VIXCLS", pct_window=252)
def vix_level(inputs: RegimeInputs, column: str = "VIXCLS", pct_window: int = 252) -> pd.Series:
